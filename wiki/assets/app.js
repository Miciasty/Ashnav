...
(() => {
  ...
  if (!config || !Array.isArray(pages) || !pages.length) {
    article.innerHTML = '<h1>Documentation is unavailable</h1><p>Check that the local content scripts listed in index.html are present and contain valid JavaScript.</p>';
    return;
  }
    ...
    if (pageChanged) {
      ...
      if (!page) {
        ...
        all('figure svg', article).forEach(svg => {
          ...
          viewport.setAttribute('aria-label', 'Diagram, scroll horizontally on narrow screens');
          const width = Number(svg.getAttribute('viewBox')?.split(/\s+/)[2]) || 560;
          svg.style.minWidth = `${Math.min(600, width)}px`;
          svg.before(viewport);
          viewport.append(svg);
        });
      }
