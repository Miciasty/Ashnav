...
const sourceVersion = pom.match(/<artifactId>ashnav<\/artifactId>\s*<version>([^<]+)<\/version>/)?.[1];
if (config.product !== 'Ashnav' || config.version !== sourceVersion || config.demo !== false) {
  errors.push('Publication identity must be Ashnav, demo=false, and the current POM version.');
}
for (const match of shell.matchAll(/href="#\/([^"?]+)"/g)) {
  if (!ids.has(match[1])) errors.push(`Shell link to unknown page ${match[1]}`);
}
let linkCount=0;
for (const page of pages) {
  const html = [page.intro || '', ...(page.sections || []).map(s=>s.html)].join('\n');
  if (/Shelter|fictional plugin|\{\{[^}]+\}\}/i.test(html)) errors.push(`${page.id}: unreplaced demonstration content.`);
  ...
  for (const match of html.matchAll(/data-diagram=["']([^"']+)["']/g)) {
    if(!['corridor','directed-graph','neighborhood','policy-detour','world-mapping','frame-capture','solver-comparison','route-playground','height-path'].includes(match[1])) errors.push(`${page.id}: unknown diagram ${match[1]}`);
  }
...
for (const match of shell.matchAll(/<script src="\.\/(assets\/[^" ]+)"/g)) {
  try { new vm.Script(await readFile(path.join(root, match[1]), 'utf8'), {filename:match[1]}); }
  catch (error) { errors.push(`Script syntax ${match[1]}: ${error.message}`); }
}
