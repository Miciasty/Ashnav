...
dependencies {
    ...
    {
      id: 'examples', category: 'Pathfinding', title: 'Examples', kind: 'guide', readingTime: 15,
      ...
      sections: [
        { id: 'tower-route', title: 'Climb a tower in 3D', html: `<p>The start is on a 16 × 16 platform at foot-cell Y = 1. The goal is on a tower roof at Y = 6. Two staircases connect these heights; collision blocks and a movable barrier restrict the approach.</p><div data-diagram="height-path"></div><p><strong>Try it:</strong> move the barrier, inspect the rising part of the height profile, then disable one-block steps. Rotate the camera to follow a route hidden behind the tower. Camera changes leave the graph and query result unchanged.</p>${note('The application supplies the movement model','<p>This example builds a <code>GridWalkabilityGraph3</code> with <code>N18</code>, then uses <code>PolicyWeightedIntGraph</code> to retain only adjacent horizontal columns with a height difference of at most one block. The application checks support and two-cell headroom and excludes obstacle roofs. Ashnav supplies the graph search; it does not infer stairs, jumps, or collision handling.</p><p>After moving the barrier, the example captures new candidate cells, creates a new graph and solver, and searches again. An existing snapshot does not update itself. This is repeated pathfinding on changed inputs, not incremental replanning or prediction of future obstacle positions.</p>')}<details class="tower-java-example"><summary>Run the same scene in Java · TowerRouteExample.java</summary>${code(window.WIKI_TOWER_JAVA,'java','TowerRouteExample.java')}</details><p>The class runs all three barrier positions and checks that the roof becomes unreachable when height changes are forbidden. Save it as <code>src/main/java/TowerRouteExample.java</code> and use the <a href="#/quick-start?section=run-example">quick-start compile and run commands</a> with this main class.</p>` },
